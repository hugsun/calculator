...
#define BIND_BUTTON(builder, input_value)                                                                             \
    {                                                                                                                 \
        ...
        g_signal_connect(button_input_##input_value, "clicked", G_CALLBACK(OnInputButtonClick), input_##input_value); \
    }

UiContext *ui_context;

void DisplayTextGtkLabel(char *text)
{
...
static void InitGtk()
{
    ...
    ui_context->calc_context->display_text = DisplayTextGtkLabel;
}
...
void OnInputButtonClick(void *widget, void *data)
{
    HandleInput(ui_context->calc_context, ((char *)data)[0]);
}
...
static void GtkMain(int argc, char *argv[])
{
    ...
    HandleInput(ui_context->calc_context, '0');
    gtk_main();
